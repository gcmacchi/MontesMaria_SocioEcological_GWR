# ============================================================================
# 03_evi_epoch2.R
# Epoch 2 (recent) pipeline: Landsat 8/9, Jan-Feb window 2021-2022.
# Download -> crop -> QA mask -> EVI (no harmonization needed -- OLI is the
# native reference sensor throughout this study).
# ============================================================================
#
# REQUIRES: 00_config.R sourced first, and 02_evi_epoch1.R already run
#           (02_evi_epoch1_results.R present at the repository root, for
#           EPOCH1_N_TARGET).
#
# Scene list source: USGS EarthExplorer export, study Path/Row.
# STAC identifiers strip the second (processing-date) timestamp present in
# USGS product IDs, same convention as 02_evi_epoch1.R.
#
# Cumulative valid-pixel matching: scenes are processed in chronological
# order; the running valid-pixel total is compared against EPOCH1_N_TARGET.
# Processing stops (for composite purposes) at the first scene whose
# cumulative total meets or exceeds that target. This equalizes the pixel
# volume contributing to each epoch's composite, rather than equalizing
# scene count or calendar-year span -- because Landsat 7 (Epoch 1) loses
# roughly 22% of pixels per scene to its permanent Scan Line Corrector
# failure, while Landsat 8/9 (Epoch 2) has no equivalent defect, matching
# on pixel count is the only comparison that is not confounded by that
# hardware asymmetry.
#
...
# Chronologically ordered -- order matters for the accumulation rule below.
epoch2_scenes <- c(
  "LC08_L2SP_009053_20210111_02_T1",
  "LC08_L2SP_009053_20210127_02_T1",
  "LC08_L2SP_009053_20210212_02_T1",
  "LC08_L2SP_009053_20210228_02_T1",
  "LC08_L2SP_009053_20210316_02_T1",
  "LC09_L2SP_009053_20220106_02_T1",
  "LC08_L2SP_009053_20220114_02_T1",
  "LC09_L2SP_009053_20220122_02_T1",
  "LC08_L2SP_009053_20220130_02_T1",
  "LC09_L2SP_009053_20220207_02_T1",
  "LC08_L2SP_009053_20220215_02_T1",
  "LC09_L2SP_009053_20220223_02_T1",
  "LC08_L2SP_009053_20220303_02_T1",
  "LC09_L2SP_009053_20220311_02_T1",
  "LC08_L2SP_009053_20220319_02_T1",
  "LC09_L2SP_009053_20220327_02_T1",
  "LC08_L2SP_009053_20230101_02_T1"
)
...
# ----------------------------------------------------------------------------
# 2-5. Per scene: crop to study area, mask QA==21824, compute EVI (native,
#      no harmonization), track cumulative valid pixels against the target.
# ----------------------------------------------------------------------------
...
write.csv(pixel_log, file.path(DIR_EVI, "epoch2_pixel_log.csv"), row.names = FALSE)

cat("\nScenes included in composite:", length(evi_list), "of", nrow(pixel_log), "\n")
cat("Final cumulative valid pixels:", cumulative_valid,
    "(target:", EPOCH1_N_TARGET, ")\n\n")

if (!target_reached) {
  cat("WARNING: target not reached with all available scenes.",
      "Composite built from all", length(evi_list), "scenes; total pixels below target.\n\n")
}

# ----------------------------------------------------------------------------
# Composite: pixel-wise mean across the scenes retained by the accumulation
# rule above (native EVI, no harmonization)
# ----------------------------------------------------------------------------
...
epoch2_mean <- mean(evi_stack, na.rm = TRUE)
names(epoch2_mean) <- "EPOCH2_EVI_MEAN"

writeRaster(epoch2_mean, file.path(DIR_EVI_OUT, "EPOCH2_EVI_MEAN.tif"),
            overwrite = TRUE, gdal = c("COMPRESS=LZW"))

cat("Composite stats:\n")
print(global(epoch2_mean, c("min","max","mean","sd"), na.rm = TRUE))

cat("\n03_evi_epoch2.R complete.\n")